...
<div class="relative inline-block">
  <Menu>
    <Menu.Button
      class={cn(_class, "-ml-2 hidden items-center whitespace-nowrap rounded p-2 md:inline-flex", classes.inactive)}
    >
      {children}
    </Menu.Button>
    <Transition leave="transition-opacity" leaveFrom="opacity-100" leaveTo="opacity-0">
      <Menu.Items
        class="absolute right-0 z-20 mt-1 max-h-64 min-w-full overflow-auto rounded-md ring-1 ring-black/5 bg-white py-1 text-sm shadow-lg dark:ring-white/20 dark:bg-neutral-800"
      >
