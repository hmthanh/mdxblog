<script>
  import { MEDIA_QUERY } from "./helper"
  ...
  $: optimization =
    attribute === "class"
      ? `var d=document.documentElement.classList;${`d.remove(${attrs.map((t) => `'${t}'`).join(",")})`};`
    ...
    forcedTheme
      ...
      : enableSystem
        ? `!function(){try {${optimization}var e=localStorage.getItem('${storageKey}');${
          ...
          }if("system"===e||(!e&&${defaultSystem})){var t="${MEDIA_QUERY}",m=window.matchMedia(t);if(m.media!==t||m.matches){${updateDOM(
            "dark"
