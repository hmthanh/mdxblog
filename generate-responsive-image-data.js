...
import {
  ...
  metadata,
} from "./src/lib/utilities/image.js";
...
sizes.forEach((sizesElement) => {
  densities.forEach((densitiesElement) =>
    outputSizes.push(densitiesElement * sizesElement),
  );
});
...
async function generateImageMeta(source) {
  ...
  const [metadataResult, dominantColourObject, placeholder] =
      ...
      const srcsetImportArray = formats.map(
        (formatsElement) =>
          `import srcset${formatsElement} from '${source}?w=${
            ...
            .join(
              ";",
            )}&format=${formatsElement === "auto" ? format : formatsElement}&as=srcset';`,
      );
      const sources = `[\n${formats
        .map(
          (formatsElement) =>
